$(document).ready(function(){
    ...
    $('#btnPerFiltro').keyup(function(){
      
        $.ajax({
            url:'index.php',
            type:'post',
            dataType:'json',
            data:{c:'Personas',a:'filtrarPersonas',valor:$(this).val()},
            success:function(res){
                $('#tbodyPer').empty();
                var cont=0;
                ...
                for (var i =0; i<res.length ;  i++) {
                    ...
                    $('#tbodyPer').append(
                        '<tr>'+
                        '<th><a href="?c=Personas&a=mostrarActividad&id='+res[i].idpersonas+'" data-toggle="tooltip" data-placement="bottom" title="Editar"><i style="color: blue;" class="bi bi-pencil-square"></i></a>'+
                        ' <a href="?c=Personas&a=eliminar&id='+res[i].idpersonas+'"  onclick="javascript:return  confirm("Seguro de eliminar");"" data-toggle=" tooltip" data-placement="bottom" title="Eliminar"><i style="color: red;" class="bi bi-trash-fill"></i></a></th>'+
                        '<th scope="row">'+cont+'</th>'+
                        '<td>'+res[i].idroles+'</td>'+
                        '<td>'+res[i].cedula+'</td>'+
                        '<td>'+res[i].nombres+'</td>'+
